...
export class TodoService {
  ...
  async createTodo(title: string, description: string, userId: number): Promise<Todo | Error> {
    ...
    if (!user.canCreateMoreTodos(currentActiveTodoCount)) {
        return new Error(`The user has exceeded the maximum limit of ALL that can be created by the plan ${user.TranslateCharPlan()}`);
    }
  ...
  async getTodos(userId: number): Promise<Todo[] | Error> {
    ...
    return await this.todoRepository.find({ where: { user: { id: userId } }, relations: ["tasks"] });
  }

  async updateTodo(id: number, title?: string, description?: string, active?: boolean): Promise<Todo | Error> {
